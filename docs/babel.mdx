...
`@emotion/babel-plugin` is highly recommended. All of the options that can be provided to `@emotion/babel-plugin` are documented in [`@emotion/babel-plugin`'s README](https://github.com/@zedvision/emotion-js/emotion/tree/master/packages/babel-plugin).
